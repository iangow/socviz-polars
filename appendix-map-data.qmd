# "Preparing Map Data" {#sec-adx-map}

This appendix contains the data-preparation code used to create the Parquet files read by the maps chapter.
...
Several map-related files are exported directly from the `socviz` R package data and from `ggplot2::map_data("state")`.
...
The `states_sf` data set starts from the `geoda.ncovr` geometry in the
`geodatasets` package.
It stores one row per state exterior-ring vertex, keeping longitude and latitude
unprojected so the map chapter can apply the Albers projection just before
plotting.
...
states_sf.write_parquet("data/states_sf.parquet")
```

The county files are built from Census cartographic county boundaries and ACS 5-year county data.
DuckDB's spatial extension handles the geometry work; Polars handles the tabular ACS data and derived variables.
...
Run this chunk when the county Parquet files need to be regenerated.
